...
April 26, 2022
-Code added to copy over sys internals tools from CD, as it was found not to work from \\live.sysinternals.com\tools if the asset wasn't joined to the domain
...
$RPath = "$CDDrive\Sysinternals"

$SysInternalsApps = ("autoruns.chm", "Autoruns.exe", "Autoruns64.dll", "Autoruns64.exe", "Autoruns64a.dll", "autorunsc.exe", "autorunsc64.exe", "Bginfo.exe", "Bginfo64.exe",  `
"CPUSTRES.EXE", "CPUSTRES64.EXE", "Diskmon.exe", "DISKMON.HLP", "Diskmon64.exe", "DiskView.exe", "DiskView64.exe", "handle.exe", "handle64.exe", "PORTMON.CNT",  `
"portmon.exe", "PORTMON.HLP", "procdump.exe", "procdump64.exe", "procexp.chm", "procexp.exe", "procexp64.exe", "procmon.chm", "Procmon.exe", "Procmon64.exe" , `
"PsExec.exe", "PsExec64.exe", "tcpview.chm ", "tcpview.exe", "TCPVIEW.HLP", "tcpview64.exe")

ForEach ($i in $SysInternalsApps) {
    
    write-host "Copying over $i from Sys internals live from $RPath"

    copy-item "$RPath\$i" -Destination "C:\Windows\System32\$i" -Force -PassThru | Set-ItemProperty -name isreadonly -Value $false
}
