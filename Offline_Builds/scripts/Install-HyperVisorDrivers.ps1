...
-Code sync for intro cosmetic stuff from Start-PostOSBuild: Setting server manager off/stopping it etc
...
IF ($VMType -eq "KVM") {   
    ...
    (Start-Process "msiexec.exe" -ArgumentList '/QB /norestart /i ""virtio-win-gt-x64.msi""' -NoNewWindow -Wait -PassThru).ExitCode
